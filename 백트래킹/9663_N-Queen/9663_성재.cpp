...
void nQueen::func(bool** arr, int cur) {   // 백트래킹 함수
	if (cur < n) {   // 검사할 행이 남았을 때
		for (int i = 0; i < n; i++) {   // 각 열에 퀸이 들어갈 수 있는지 검사
			is_empty = true;
			for (int j = 0; j < cur; j++) {   // 현재 행보다 앞에 행에서 퀸이 겹치는지 검사
			...
			if (is_empty) {    // 세가지 경우 모두 없었을 경우
				arr[cur][i] = true;   // 퀸 배치
				func(arr, cur + 1);   // 다음 행 검사
